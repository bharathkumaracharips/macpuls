...
export const TopHeader: React.FC = () => {
  ...
  return (
    <header className="h-16 border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-md px-6 flex items-center justify-between select-none">
      {/* Title */}
      <div>
      ...
      <div className="flex items-center gap-4">
        ...
        {metrics && (
          <div className="flex items-center gap-3 bg-slate-950/60 border border-slate-800/80 rounded-full px-3.5 py-1 text-xs text-slate-300 font-mono">
            ...
            <div className="flex items-center gap-1.5" title="CPU Temperature">
              ...
              <span>{metrics.thermal.cpu_temp_c ? `${metrics.thermal.cpu_temp_c.toFixed(1)}°C` : 'N/A'}</span>
            </div>
            <div className="w-px h-3 bg-slate-800" />
        ...
        <button
          ...
          className="flex items-center gap-2 bg-slate-800/80 hover:bg-slate-700/80 text-slate-300 hover:text-white px-3 py-1.5 rounded-lg border border-slate-700/60 text-xs transition-all duration-150"
        >
